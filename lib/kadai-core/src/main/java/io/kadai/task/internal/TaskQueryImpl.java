...
public class TaskQueryImpl implements TaskQuery {
  ...
  private static final String LINK_TO_COUNTER_DB2 =
      "io.kadai.task.internal.TaskQueryMapper.countQueryTasksDb2";
  private static final String LINK_TO_VALUE_MAPPER =
      "io.kadai.task.internal.TaskQueryMapper.queryTaskColumnValues";
  ...
  TaskQueryImpl(InternalKadaiEngine kadaiEngine) {
  ...
  public TaskQuery groupByPor() {
    if (kadaiEngine.getEngine().getConfiguration().isUseSpecificDb2Taskquery()
        && getDB().equals(DB.DB2)) {
      throw new SystemException(
          "kadai.feature.useSpecificDb2Taskquery needs to be set to false "
  ...
  public ObjectReferenceQuery createObjectReferenceQuery() {
    return new ObjectReferenceQueryImpl(kadaiEngine);
  }
  ...
  public TaskSummary single() {
    ...
    try {
      ...
      TaskSummaryImpl taskSummaryImpl =
          kadaiEngine.getSqlSession().selectOne(getLinkToMapperScript(), this);
  ...
  public String toString() {
    return "TaskQueryImpl [kadaiEngine="
        + kadaiEngine
        + ", taskService="
        + taskService
        + ", orderByOuter="
